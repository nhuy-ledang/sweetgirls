...
use Modules\User\Repositories\GroupRepository;
...
class GroupController extends ApiBaseModuleController {
    ...
    /**
     * @OA\Get(
     *   path="/backend/user_group_all",
     *   summary="Get User Group All",
     *   operationId="getUserGroupAll",
     *   tags={"BackendUserGroups"},
     *   security={{"bearer":{}}},
     ...
     *     )
     *   ),
     *   @OA\Parameter(name="id", description="User Group Id", in="path", example=1),
     *   @OA\Response(response=200, description="Success", @OA\JsonContent()),
     *   @OA\Response(response=400, description="Invalid request params"),
     *   @OA\Response(response=401, description="Caller is not authenticated"),
     *   @OA\Response(response=404, description="Resource not found"),
    ...
    public function destroy($id) {
        try {
            $model = $this->model_repository->getModel()->where('id', '<>', 1)->where('id', $id)->first();
            if (!$model) return $this->errorNotFound();
