...
use Modules\Product\Repositories\ProductBestsellerRepository;
 ...
 * Date: 2022-06-08
 */
class ProductBestsellerController extends ApiBaseModuleController {
    /**
     * @var \Modules\Product\Repositories\ProductRepository
     */
    protected $product_repository;

    public function __construct(Request $request, ProductBestsellerRepository $product_bestseller_repository, ProductRepository $product_repository) {
        $this->model_repository = $product_bestseller_repository;
    ...
    /**
     * @OA\Get(
     *   path="/backend/pd_product_bestsellers",
     *   summary="Get Product Bestsellers",
     *   operationId="pdGetProductBestsellers",
     *   tags={"BackendPdProductBestsellers"},
     *   security={{"bearer":{}}},
    ...
    public function index() {
        try {
            ...
            $results = $this->setUpQueryBuilder($this->product_repository->getModel(), $queries, false, $fields)
                ->rightJoin('pd__product_bestsellers as lat', 'product_id', '=', 'pd__products.id')
            ...
            if (!$paging) {
                ...
                $totalCount = $this->setUpQueryBuilder($this->product_repository->getModel(), $queries, true)
                    ->rightJoin('pd__product_bestsellers as lat', 'product_id', '=', 'pd__products.id')->count();
     ...
     * @OA\Post(
     *   path="/backend/pd_product_bestsellers",
     *   summary="Create Product Bestseller",
     *   operationId="pdCreateProductBestseller",
     *   tags={"BackendPdProductBestsellers"},
     *   security={{"bearer":{}}},
     *   @OA\RequestBody(
     *     @OA\JsonContent(
    ...
    /**
     * @OA\Delete(
     *   path="/backend/pd_product_bestsellers/{id}",
     *   summary="Delete Product Bestseller",
     *   operationId="pdDeleteProductBestseller",
     *   tags={"BackendPdProductBestsellers"},
     *   security={{"bearer":{}}},
     *   @OA\Parameter(name="id", in="path", description="Product Bestseller Id", example=1),
     *   @OA\Response(response=200, description="Success", @OA\JsonContent()),
     *   @OA\Response(response=400, description="Invalid request params"),
     *   @OA\Response(response=401, description="Caller is not authenticated"),
     *   @OA\Response(response=404, description="Resource not found"),
