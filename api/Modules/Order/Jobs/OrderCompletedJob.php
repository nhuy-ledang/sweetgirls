...
class OrderCompletedJob implements ShouldQueue {
    ...
    public function handle() {
        ...
        // Send to customer
        if (!empty($email)) $this->adapter->send('order::mail/order_completed', $data, function($message) use ($email, $order) {
            $message->to($email)->subject("Đơn hàng {$order->no} đã được giao thành công");
        });
        // Send alert to admin
