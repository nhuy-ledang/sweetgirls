angular.module('app.dlgBankNumber', [])

.controller('DlgBankNumberCtrl', function($scope, $rootScope, $uibModalInstance, affiliates) {
  var vm = this;
  ...
  vm.params = {
    bank_number: $rootScope.affInfo.bank_number,
  };
  ...
  $scope.submit = function(form) {
    ...
    if(form.$valid) {
      ...
      affiliates.updateAffiliate(params, true).then(function(res) {
        ...
        $rootScope.$emit('dlgBankNumberParams', params);
        $rootScope.affInfo.bank_number = params.bank_number;
...
.factory('dlgBankNumber', function($uibModal, helper) {
  return {
    show: function(fn, data) {
      return $uibModal.open({
        animation: true,
        //backdrop: 'static',
        templateUrl: 'pages/affiliate/dlgBankNumber/dlgBankNumber.tpl.html',
        controller: 'DlgBankNumberCtrl',
        controllerAs: 'vm',
        size: 'sm modal-edit-bank-number modal-dialog-centered',
        resolve: {
          $data: function() {
            return data;
          }
