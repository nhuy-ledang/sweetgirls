angular.module('app.dlgIDAddress', [])

.controller('DlgIDAddressCtrl', function($scope, $rootScope, $uibModalInstance, affiliates) {
  var vm = this;
  ...
  vm.params = {
    address: $rootScope.affInfo.address,
  };
  ...
  $scope.submit = function(form) {
    ...
    if (form.$valid) {
      ...
      affiliates.updateAffiliate(params, true).then(function(res) {
        ...
        $rootScope.$emit('dlgIDAddressParams', params);
        $rootScope.affInfo.address = params.address;
...
.factory('dlgIDAddress', function($uibModal, helper) {
  return {
    show: function(fn, data) {
      return $uibModal.open({
        animation: true,
        //backdrop: 'static',
        templateUrl: 'pages/affiliate/dlgIDAddress/dlgIDAddress.tpl.html',
        controller: 'DlgIDAddressCtrl',
        controllerAs: 'vm',
        size: 'sm modal-edit-id-address modal-dialog-centered',
        resolve: {
          $data: function() {
            return data;
          }
