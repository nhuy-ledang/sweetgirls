...
class Image {
    ...
    public function resizeCrop($width = 0, $height = 0, $align = 'c') {
        if (!$this->width || !$this->height) {
            return;
        }
        ...
        $cmp_x = $this->width / $width;
        $cmp_y = $this->height / $height;
        ...
        imagecopyresampled($this->image, $image_old, $origin_x, $origin_y, $src_x, $src_y, $width, $height, $src_w, $src_h);
        imagedestroy($image_old);
     ...
     * @param int $y
     */
	public function watermark($watermark, $position = 'bottomright', $x = 0, $y = 0) {
		switch($position) {
			case 'topleft':
				$watermark_pos_x = 0;
				$watermark_pos_y = 0;
			...
			case 'bottomcenter':
				...
				break;
			case 'bottomright':
				$watermark_pos_x = $this->width - $watermark->getWidth() - $x;
				$watermark_pos_y = $this->height - $watermark->getHeight() - $y;
				break;
		}

        if ($this->width - $watermark->getWidth() >= $watermark->getWidth() * 2) {
