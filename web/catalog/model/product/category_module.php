...
class ModelProductCategoryModule extends ModelPageModuleBase {
    protected $table = DB_PREFIX . 'pd__category_modules';
    protected $desc_table = DB_PREFIX . 'pd__category_module_desc';
    protected $fields = [];
    ...
    public function getModules($category_id) {
        $fields = ['md.*'];
        if ($this->config->get('config_language') != $this->config->get('language_code_default')) {
            $fields[] = 'd.name as d__name';
            $fields[] = 'd.title as d__title';
        ...
        $implode = [];
        $implode[] = "md.category_id = '" . (int)$category_id . "'";
        $implode[] = "md.status = 1";
        if (!empty($implode)) $sql .= " where " . implode(' and ', $implode);
