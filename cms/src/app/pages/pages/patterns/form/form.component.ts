...
import { CategoriesRepository, LayoutPatternsRepository, ModulesRepository, PagesRepository } from '../../shared/services';
import { BaseModuleFormComponent } from '../../modules/form/base.form.component';
import { DlgCategorySelectComponent } from '../../shared/modals';

@Component({
  selector: 'ngx-pg-pattern-form',
  templateUrl: './form.component.html',
})

export class PatternFormComponent extends BaseModuleFormComponent implements OnInit, OnDestroy {
  ...
  constructor(fb: FormBuilder, router: Router, security: Security, state: GlobalState, repository: LayoutPatternsRepository, languages: LanguagesRepository,
  ...
  show(info?: any): void {
    ...
    /*if (info) {
      ...
      _.each(this.controls, (val, key) => {
        if (this.controls.hasOwnProperty(key) && this.controls[key] instanceof FormControl && !_.includes(['is_overwrite'], key)) this.controls[key].setValue('');
      });
