...
import { CategoriesRepository, LayoutModulesRepository, ModulesRepository, PagesRepository } from '../../../shared/services';
import { BaseModuleFormComponent } from '../../../modules/form/base.form.component';
import { DlgCategorySelectComponent } from '../../../shared/modals';

@Component({
  selector: 'ngx-pg-ly-module-form',
  templateUrl: './form.component.html',
})

export class LayoutModuleFormComponent extends BaseModuleFormComponent implements OnInit, OnDestroy {
  ...
  constructor(fb: FormBuilder, router: Router, security: Security, state: GlobalState, repository: LayoutModulesRepository, languages: LanguagesRepository,
  ...
  protected getListModules(): void {
    this.repository.get({data: {layout_id: this.info.layout_id}}).then((res: any) => {
  ...
  show(info: any): void {
    ...
    setTimeout(() => this.getListModules(), 1500);
    this.modal.show();
  }
  ...
  onSubmit(params: any, dontHide?: boolean, loading ?: boolean): void {
    ...
    if (this.form.valid) {
      ...
      if (this.info) {
        ...
        /*newParams['layout_id'] = this.layout.id;
