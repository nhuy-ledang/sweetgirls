...
import { LayoutModulesRepository } from '../../shared/services';
import { LayoutModuleFormComponent } from './form/form.component';
import { DlgModuleSelectComponent, DlgPatternSelectComponent } from '../../shared/modals';

@Component({
  selector: 'ngx-pg-ly-modules',
  templateUrl: './modules.component.html',
})
export class LayoutModulesComponent extends AppList implements OnInit, OnDestroy, AfterViewInit {
  repository: LayoutModulesRepository;
  @ViewChild(ConfirmComponent) confirm: ConfirmComponent;
  @ViewChild(LayoutModuleFormComponent) form: LayoutModuleFormComponent;
  @ViewChild(DlgModuleSelectComponent) dlgModule: DlgModuleSelectComponent;
  info: any;

  @Input() set layout(item: any) {
    this.info = item;
    this.data.data.layout_id = item.id;
    this.getData();
  }

  constructor(router: Router, security: Security, state: GlobalState, repository: LayoutModulesRepository, private _dialog: Dialog) {
    super(router, security, state, repository);
    this.data.sort = 'name';
    this.data.order = 'asc';
    this.data.data = {layout_id: 0, embed: 'descs,module'};
  }
  ...
  onModuleSelect(res: any[]): void {
    ...
    if (ids.length) this.repository.cloneModules({layout_id: this.info.id, ids: ids}, true).then((res) => {
      console.log(res.data);
      this.data.page = 1;
      this.getData();
    }, (res: any) => {
  ...
  onPatternSelect(res: any[]): void {
    const ids = [];
    _.forEach(res, (item: any) => ids.push(item.id));
    if (ids.length) this.repository.clonePatterns({layout_id: this.info.id, ids: ids}, true).then((res) => {
      console.log(res.data);
      this.data.page = 1;
      this.getData();
    }, (res: any) => {
