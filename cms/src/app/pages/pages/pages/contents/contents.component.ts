...
import { PageContentFormComponent } from './form/form.component';
import { DlgModuleSelectComponent, DlgPatternSelectComponent } from '../../shared/modals';

@Component({
  selector: 'ngx-pg-pg-contents',
  templateUrl: './contents.component.html',
})
export class PageContentsComponent extends AppList implements OnInit, OnDestroy, AfterViewInit {
  repository: PageContentsRepository;
  @ViewChild(ConfirmComponent) confirm: ConfirmComponent;
  @ViewChild(PageContentFormComponent) form: PageContentFormComponent;
  @ViewChild(DlgModuleSelectComponent) dlgModule: DlgModuleSelectComponent;
  info: any;

  @Input() set page(item: any) {
    this.info = item;
    this.data.data.page_id = item.id;
    this.getData();
  }

  constructor(router: Router, security: Security, state: GlobalState, repository: PageContentsRepository, private _dialog: Dialog) {
    super(router, security, state, repository);
    this.data.sort = 'name';
    this.data.order = 'asc';
    this.data.data = {page_id: 0, embed: 'descs,module'};
  }
