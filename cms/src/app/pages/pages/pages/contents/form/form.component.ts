...
import { CategoriesRepository, ModulesRepository, PageContentsRepository, PagesRepository } from '../../../shared/services';
import { BaseModuleFormComponent } from '../../../modules/form/base.form.component';
import { DlgCategorySelectComponent, DlgModuleSelectComponent, DlgWidgetSelectComponent } from '../../../shared/modals';

@Component({
  selector: 'ngx-pg-pg-content-form',
  templateUrl: './form.component.html',
})
export class PageContentFormComponent extends BaseModuleFormComponent implements OnInit, OnDestroy {
  ...
  constructor(fb: FormBuilder, router: Router, security: Security, state: GlobalState, repository: PageContentsRepository, languages: LanguagesRepository,
  ...
  show(info: any): void {
    ...
    /*setTimeout(() => {
      this.getListModules(this.page.id, info);
    }, 1500);*/
    this.modal.show();
  }
  ...
  onSubmit(params: any, dontHide?: boolean, loading ?: boolean): void {
    ...
    if (this.form.valid) {
      ...
      // newParams.style = newParams.code;
      // if (params.is_overwrite && this.moduleSelected) newParams.style = this.moduleSelected.code;
      ...
      if (this.info) {
        ...
        /*newParams['page_id'] = this.page.id;
  ...
  @ViewChild(DlgWidgetSelectComponent) dlgWidget: DlgWidgetSelectComponent;

  openWidgets(): void {
    this.dlgWidget.show();
  }

  onWidgetSelect(res: {id: string, item: any, cl: any}): void {
    if (res) {
      console.log(res);
      this.controls.tile.setValue(res.id);
      this.widgetPreview = res.cl.preview;
    }
  }
}
