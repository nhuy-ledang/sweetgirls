...
import { CategoryPropertyFormComponent } from './form/form.component';
import { CategoriesRepository, CategoryPropertiesRepository } from '../../shared/services';

@Component({
  selector: 'ngx-pd-category-properties',
  templateUrl: './properties.component.html',
})
export class CategoryPropertiesComponent extends AppList implements OnInit, OnDestroy, AfterViewInit {
  repository: CategoriesRepository;
  @ViewChild(ConfirmComponent) confirm: ConfirmComponent;
  @ViewChild(CategoryPropertyFormComponent) form: CategoryPropertyFormComponent;
  info: any;
  ...
  @Input() set category(item: any) {
    console.log(item);
    this.info = item;
    this.data.items = this.info.propertiess;
  ...
  onChoiceSuccess(ids: []): void {
    console.log(ids);
    if (ids.length) {
      this._categories.createProperties(this.data.data.category_id, {ids: ids.join(',')}, true).then((res: any) => {
          this.data.page = 1;
          this.getData();
        }, (res) => {
