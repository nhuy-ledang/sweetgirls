...
import { CategoryModuleFormComponent } from './form/form.component';
import { DlgModuleSelectComponent, DlgPatternSelectComponent } from '../../../pages/shared/modals';

@Component({
  selector: 'ngx-pd-category-modules',
  templateUrl: './modules.component.html',
})
export class CategoryModulesComponent extends AppList implements OnInit, OnDestroy, AfterViewInit {
  repository: CategoryModulesRepository;
  @ViewChild(ConfirmComponent) confirm: ConfirmComponent;
  @ViewChild(CategoryModuleFormComponent) form: CategoryModuleFormComponent;
  ...
  constructor(router: Router, security: Security, state: GlobalState, repository: CategoryModulesRepository, private _dialog: Dialog) {
    super(router, security, state, repository);
    this.data.sort = 'name';
    this.data.order = 'asc';
    this.data.data = {category_id: 0, embed: 'descs,module'};
  }
  ...
  onModuleSelect(res: any[]): void {
    ...
    if (ids.length) this.repository.cloneModules({category_id: this.info.id, ids: ids}, true).then((res) => {
      console.log(res.data);
      this.data.page = 1;
      this.getData();
    }, (res: any) => {
  ...
  onPatternSelect(res: any[]): void {
    const ids = [];
    _.forEach(res, (item: any) => ids.push(item.id));
    if (ids.length) this.repository.clonePatterns({category_id: this.info.id, ids: ids}, true).then((res) => {
      console.log(res.data);
      this.data.page = 1;
      this.getData();
    }, (res: any) => {
